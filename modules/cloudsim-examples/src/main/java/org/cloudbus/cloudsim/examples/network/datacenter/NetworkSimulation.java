...
public class NetworkSimulation extends AppCloudlet {
    ...
    public static final ArrayList<Integer> VM_IDS = new ArrayList<Integer>();
    public static final int MAX_NUM_VMS = 5;
    static {
        for (int idx = 0; idx < MAX_NUM_VMS;idx++){
            VM_IDS.add(new Integer(idx));
        }
    }
    /** The vmlist. */
    ...
    public NetworkSimulation(int type, int appID, double deadline, 	int userId) {
        super(type, appID, deadline, MAX_NUM_VMS,userId);
    ...
    public static void main(String[] args) {
        ...
        Log.printLine("Starting NetworkSimulation...");
        int numApps = 1500;
        if (args.length > 0) numApps = Integer.parseInt(args[0]);
        NetworkConstants.MAX_NUM_APPS = numApps;
        System.out.println("Num apps:" + NetworkConstants.MAX_NUM_APPS);
        ...
        try {
            ...
            NetworkDatacenter nwDatacenter1 = createDatacenter("NetworkSimulation_Datacenter-1");

            NetworkDatacenter nwDatacenter2 = createDatacenter("NetworkSimulation_Datacenter-2");
...
//            broker.setLinkDC(nwDatacenter2);
            ...
            //NetworkSimulation networkSimulation = new NetworkSimulation(0,24,1000.00,MAX_NUM_VMS);
            ...
            Log.printLine("Network Simulation finished!");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void createCloudletList(List<Integer> vmIdList){
        ...
        for (int idx =0; idx < MAX_NUM_VMS ;idx++){
            ...
            for(int i = 0;i < MAX_NUM_VMS;i++) {
                ...
                int stgId = 0;
                for (int jIdx = 0;jIdx < MAX_NUM_VMS;jIdx++){
                    if (idx != jIdx) {
