...
public class CloudSim {
	...
	public static List<SimEntity> getEntityList() {
		...
		return (List<SimEntity>)Collections.unmodifiableCollection(entities);
	}
	...
	public static boolean runClockTick() {
        for (SimEntity entity : entities) {
            if (entity.getState() == SimEntity.RUNNABLE) {
                entity.run();
            }
        }

		// If there are more future events then deal with them
		boolean isEmptyQueue = future.size() == 0;
		if (!isEmptyQueue) {
			List<SimEvent> toRemove = new ArrayList<SimEvent>();
			Iterator<SimEvent> iterator = future.iterator();
			SimEvent first = iterator.next();
            double firstEventTime = first.eventTime();
            processEvent(first);
            iterator.remove();

			// Check if next events are at same time...
			boolean trymore = iterator.hasNext();
			while (trymore) {
				SimEvent next = iterator.next();
				if (next.eventTime() == firstEventTime) {
					processEvent(next);
					toRemove.add(next);
                    trymore = iterator.hasNext();
                    continue;
				}
	...
	private static void processEvent(SimEvent e) {
		...
		switch (e.getType()) {
			...
			case SimEvent.SEND:
				...
				if (dest >= 0) {
					int tag = e.getTag();
                    SimEntity destEvent = entities.get(dest);
					if (destEvent.getState() == SimEntity.WAITING) {
						Predicate p = waitPredicates.get(dest);
						if ((p == null) || (tag == 9999) || (p.match(e))) {
							destEvent.setEventBuffer((SimEvent) e.clone());
							destEvent.setState(SimEntity.RUNNABLE);
                ...
                else throw new IllegalArgumentException("Attempt to send to a null entity detected.");
				break;
