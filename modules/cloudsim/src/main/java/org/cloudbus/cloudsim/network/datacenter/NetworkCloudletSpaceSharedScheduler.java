...
public class NetworkCloudletSpaceSharedScheduler extends CloudletScheduler {
    ...
    private void printPktStats(NetworkCloudlet cl,HostPacket pkt) {
        double latency = (pkt.recievetime - pkt.sendtime);
        double throughput = pkt.data;

        if (latency != 0) throughput = pkt.data/latency;

        String logMsg = "From:" + pkt.sender + " To:" + pkt.reciever;
                logMsg += " Latency:" + latency + " Throughput:" + throughput;
	...
	private void changetonextstage(NetworkCloudlet cl, TaskStage st) {
        ...
        if (st.type == NetworkConstants.WAIT_RECV) NetworkHost.processPacket(st.currPkt);
	}
