...
public class NetworkCloudlet extends Cloudlet implements Comparable<Object> {
	...
	private static final Logger LOGGER = Logger.getLogger(NetworkCloudlet.class);
    ...
    public ArrayList<PacketStats> pktStatsList = new ArrayList<PacketStats>();
    ...
    public static final int MAX_NUM_PKTS = 100;
    public void logPacket(PacketStats pkt){
		LOGGER.info(pkt.statString());

        /*
        if (pktStatsList.size() < MAX_NUM_PKTS){
            pktStatsList.add(pkt);
        }else{
            Collections.shuffle(pktStatsList,rnd);
            Iterator<PacketStats> itr = pktStatsList.iterator();
            while(itr.hasNext())
            {
                PacketStats curr = itr.next();
            }
            Log.printLine(pkt.statString());
            pktStatsList = null;
            pktStatsList = new ArrayList<PacketStats>();
        }
        */
