...
public class NetworkTopology {
    ...
    public static String getTopologyData() {
        String result = "";
        if (map != null){
            int nodes = graph.getNumberOfNodes();
            double[][] mtx = new double[nodes][nodes];
            ...
            while (iter.hasNext()) {
                TopologicalLink edge = iter.next();
                int src = edge.getSrcNodeID();
                int dst = edge.getDestNodeID();
                double bw = edge.getLinkBw();
                double lat = edge.getLinkDelay();
                result += "From:" + src + "---> To:" + dst + " Bandwidth:"  + bw + " Latency:" + lat + "\n";
            }
        }
        return result;
    }
